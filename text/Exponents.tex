...
\noindent
Both of these come from rule \ref{eqn:mult_exponents}. Additionally, you might see $x^{1/a}$ written as $\sqrt[a]{x}$. Mathematicians call $\sqrt[a]{x}$ the \emph{radical} form. Both of these have the same meaning, they just look a bit different. Anytime you see $\sqrt[a]{x}$, you can replace it with $x^{1/a}$ and vice versa.
...
If we use the second version, $(8^{1/3})^2$, we would start by finding the cube root of 8. When we factor, we get $8=2\times2\times2$, which show us that $2=8^{1/3}$. This gives us $(8^{1/3})^2 = (2)^2 = 4$.}\\
