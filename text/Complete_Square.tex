...
\noindent
This tells us that $b$ influences our constant term. We want the constant terms to match, so we have $6=a^2+b$. We know $a=4$, so really we have $6=4+b$, giving us that $b=2$. That means we have $f(x)=(x+2)^2+2$.}\\
...
We've now found all three parameters, so we are done and have that $f(x)=4(x+\frac{3}{2})^2-12$.}\\
