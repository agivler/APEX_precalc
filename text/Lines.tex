...
There are two common ways of expressing linear functions, point-slope form and slope-intercept form. As you can probably guess from the names, both of these forms will require that you first know the slope. Slope-intercept form requires knowing the y-intercept of the function, and point-slope form allows you to use any point on the line. It's good to comfortable working with both forms and with switching between them because sometimes one form will be more useful than the other. In calculus, point-slope form often comes in handy because we will be looking at lines over a small region that won't always contain the y-intercept of the line. Slope-intercept form is:
...
\example{ex_line_forms}{Writing an Equation for a Line}{Write the equation for the line that passes through $(2,4)$ and is parallel to $3x+y=6$ in slope-intercept form.}{Let's analyze the information we have to work with. First, we know that our line goes through the point $(2,4)$. We know we also need its slope.

We're told it's parallel to $3x+y=6$, so it will have the same slope as that line has. However, this line isn't in either of our forms, so it's not immediately clear what the slope is. We'll start by putting this line, $3x+y=6$ into slope-intercept form (because it requires less algebra than putting it into point-slope form would). To put it into slope-intercept form, we need to isolate $y$. We'll do that by subtracting $3x$ from both sides. That gives
...
\noindent
If we rearrange the right side, we get $y=-3x+6$. Now, it's it slope-intercept form and we can see that the slope is $-3$.\\
...
\noindent
Now, we'll use algebra to rewrite it into slope-intercept form (i.e., we'll isolate y):
...
\noindent
So, in slope-intercept form, the line is $y=-3x+10$. This also tells us that the y-intercept of this line is $(0,10)$.}\\
...
\noindent
and that this statement holds for every $(x,y)$ pair that are on the line. We know $m$, but are missing $b$. However, we do know that $(2,4)$ is on the line, so 
...
\noindent
Now, we know $b$, so our final answer, in slope-intercept form, is $y=-3x+10$.}\\
