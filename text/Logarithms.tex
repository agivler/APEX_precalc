...
\begin{enumerate}
	\item For this statement, we are answering the question ``9 is 3 raised to what power?'' We can see through a little bit of trial and error that $3^2=9$, so we get that $\log_3{(9)}=2$.
	\item For this statement, we are answering the question ``64 is 4 raised to what power?'' We get that $4^3=64$, so we get that $\log_4{(64)}=3$.
	\item For this statement, we are answering the question ``$\frac{1}{2}$ is 2 raised to what power?'' This one is a bit tricky. We know that $\frac{1}{2}$ is smaller than 2, so the answer to our question cannot be a positive number. If we try zero, we have that $2^0=1$, so it can't be zero either. This leaves negative numbers. If we try $2^{-1}$, we get $\frac{1}{2}$. This tells us that $\log_2{\Big( \frac{1}{2} \Big)} = -1$.
	\item For this statement, we are answering the question ``2 is 2 raised to what power?'' We can quickly see that $2^1=2$, so we get that $\log_2{(2)}=1$.
	\item For this statement, we are answering the question ``10000 is 10 raised to what power?'' We get that $10^4=10000$, so we get that $\log_{10}{(10000)}=4$.
	\item For this statement, we are answering the question ``$\frac{1}{9}$ is 3 raised to what power?'' This is similar to $\log_2{\Big(\frac{1}{2} \Big)}=-1$; here we get that $\log_3{\Big(\frac{1}{9})}=-2$ since $3^{-2} = \frac{1}{9}$.
\end{enumerate}
}\\
...
\begin{equation*}
	...
	\end{split}
\end{equation*}
Notice that when the 3 on the second term came inside, we were careful to apply it as an exponent to everything inside of the logarithm, and not just the $y$. 
}\\
...
\example{ex_split_log}{Splitting Logarithms}{Split $\displaystyle \ln{\Bigg( \frac{2x^3y^3}{wz^5}\Bigg)}$ into the sum and/or difference of multiple logarithms.}{Here, we want to rewrite as many simpler logarithms. First, we see that the logarithms has a quotient inside, so we can use the third rule to split it:
...
\example{ex_solving_log}{Solving a Logarithmic Statement}{Solve $\log_5{(2x+3)}=2$ for $x$.}{Here, the logarithm is already isolated on one side, so we can start by taking each side and using it as an exponent on a base of 5. Then, we will use the definition of logarithms shown in equation \ref{eqn:log_def} to remove the logarithm from our equation.
